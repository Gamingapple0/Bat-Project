...
function Workouts(props){
    ...
    switch(day){
        case 0:
            ...
            break
        case 1:
            workoutMuscle = 'biceps'
            break
        case 2:
            workoutMuscle = 'triceps'
            break
        case 3:
            workoutType = 'cardio'
            workoutMuscle = ''
    ...
    const cardClicked = (idFunc)=>{
        console.log('a')
        console.log(idFunc)
        const card  = document.querySelector("#workout_" + idFunc)
        console.log(card)
        const workout_info = card.querySelector(':scope > .workout-info')
        if (workout_info.classList.contains('workout-info-animate')){
        workout_info.classList.add('workout-info-animate-reverse')
        workout_info.classList.remove('workout-info-animate')
        }
        else{
        workout_info.classList.remove('workout-info-animate-reverse')
        workout_info.classList.add('workout-info-animate')
    ...
    const generateRandomWorkouts = (workoutsRequired=3) => {
      return new Promise(async (resolve, reject) => {
        try{
          const arr = await fetchWorkouts()
          var newArr = []
          let i = 0;
          do{
            ...
            if (newArr.includes(arr[randomIndex])){}
              ...
              const randWorkout = arr[randomIndex];
              newArr.push(randWorkout)
              i++
            }
          } 
          while(i < 3)
    ...
    const getRandomWorkouts = () => {
        var workoutCount = 0;
        const workouts = generateRandomWorkouts().then((workoutArray)=>{
        const customWorkoutPlan = workoutArray.map((data) => {
            workoutCount++;

            return (    
              <div onClick={() => cardClicked(data.name.replace(/\s/g, '_'))}>
                <h3>Workout {workoutCount}</h3>
                <div className="workout-card" id={'workout_' + data.name.replace(/\s/g, '_')}>
                  <div className="workout-card-general">
                    <h4>{data.name}</h4>
                  </div>
                  <div className="workout-info">
                    <div className="workout-info-extra">
                      <div>
                        {data.instructions}
                      </div>
                    </div>
          ...
          console.log(customWorkoutPlan)
        setWorkoutPlan(customWorkoutPlan.slice(0,3))
        }
        )
    ...
    return(
        ...
        <div className="workout-container">
            <div className="workout-plan-container">
              <h2 className="title">Your Custom Workout Plan For Today</h2>
        ...
        <br></br>
        <br></br>
          <div onClick={getRandomWorkouts}>
          <button  type="button" className="submit-button">
              <span className="submit-button__text">Generate Workout Plan for Today</span>
            </button>
          </div>
            <br></br>
        <br></br>
